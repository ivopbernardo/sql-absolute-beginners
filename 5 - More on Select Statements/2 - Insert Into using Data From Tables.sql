/* Another cool thing we can do is that we can do insert into statements using data 
from other tables. */
...
For example, Angela appears twice, Burt appears three times
and most first names are repeated.

Imagine that we would like to have  only the distinct values from this column - we can
...
-- first_names table is now empty 
select * from sakila.first_names;

-- We can use the query as instead of the values in the INSERT INTO command!
...
Let's create a table that will contain only the pg_movies */
...
Imagine writing all the descriptions of the movies manually in a insert into clause!*/