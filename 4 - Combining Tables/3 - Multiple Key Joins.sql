-- Let's see an example of multiple key joins:
...
-- Should we just join by customer ID? Let's see
...
-- Why do we have so many rows? Because we've produced duplicates!

-- Duplicates are one of the most common mistakes beginners make when doing queries
-- in SQL 

-- As we have several lines for the same customer, these will be duplicated because
-- each line will match 2 times with the right table for customer_id = 1 and three times for customer_id = 2 
...
-- customer ID_2 is reapeat 4 times in the customer_month 
...
-- How to solve this? The primary keys of our table are Customer ID + Month so we can use a multiple column join!
select a.customer_id, a.customer_name, b.month, b.balance
from sandbox.customer_month as a
inner join sandbox.customer_balance as b
